<template>
    <v-dialog v-model="dialog" max-width="600" @keydown.esc="close">
        <template v-slot:default="dialog">
            <v-card>
                <v-toolbar color="red" :dark="true" elevation="0" class="text-h6">{{ $t("payment_failed") }}</v-toolbar>
                <v-card-text class="text-center px-12 pt-12 pb-5">
                    ...
                    <v-btn color="primary" elevation="0" large class="px-7" @click.native="tryAgain">{{
                        $t("try_again")
                    }}</v-btn>
                </v-card-text>
